...
;layui.define(["jquery", "layer", "laytpl", "upload"], function(i) {
    var a = "2.0.85"
      , e = layui.jquery
      , t = layui.layer
      , l = layui.laytpl
      ...
      , r = function() {
        ...
        e("body").on("click", "*[layim-event]", function(i) {
            ...
            Y[t] ? Y[t].call(this, a, i) : ""
        })
    }
    ;
    r.prototype.config = function(i) {
        var a = [];
        return layui.each(Array(4), function(i) {
        ...
        window.JSON && window.JSON.parse ? (C(i),
        this) : void 0
    }
    ,
    r.prototype.on = function(i, a) {
        return "function" == typeof a && (o[i] ? o[i].push(a) : o[i] = [a]),
        this
    }
    ,
    ...
    r.prototype.chat = function(i) {
        return window.JSON && window.JSON.parse ? (I(i),
        this) : void 0
    }
    ,
    r.prototype.setChatMin = function() {
        return L(),
        this
    }
    ,
    ...
    layui.laytpl.content = function(i) {
        ...
        return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/\s{2}/g, "&nbsp").replace(/img\[([^\s]+?)\]/g, function(i) {
            ...
            return '<img alt="' + a + '" title="' + a + '" src="' + R[a] + '">'
    ...
    ;
    var f, p, h, v, g, b = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.laytpl.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.laytpl.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.laytpl.content(d.content||"&nbsp") }}</div>', "</li>"].join(""), x = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>', k = function(i, a, l) {
        return i = i || {},
        e.ajax({
            url: i.url,
            type: i.type || "get",
            ...
            cache: !1,
            success: function(i) {
                0 == i.code ? a && a(i.data || {}) : t.msg(i.msg || (l || "Error") + ": LAYIM_NOT_GET_DATA", {
                    time: 5e3
                })
            },
            error: function(i, a) {
    ...
    , w = {
        ...
        return w = e.extend(w, n),
        i.brief ? layui.each(o.ready, function(i, a) {
            a && a(n)
        }) : void k(i.init, function(a) {
            var t = i.mine || a.mine || {}
              , n = layui.data("layim")[t.id] || {}
              , s = {
                base: i,
                local: n,
                mine: t,
            ...
            S(l(u).render(s)),
            (n.close || i.min) && j(),
            layui.each(o.ready, function(i, a) {
                a && a(s)
            })
        }, "INIT")
    }
    , S = function(i) {
        return t.open({
            ...
            content: i,
            success: function(i) {
                var a = layui.data("layim")[w.mine.id] || {}
                  , l = a.skin;
                f = i,
                f.css({
                    "background-image": l ? "url(" + l + ")" : "none"
                }),
                w.base.right && i.css("margin-left", "-" + w.base.right),
                p && t.close(p.attr("times"));
                var n = []
                  , s = i.find(".layim-list-history");
                s.find("li").each(function() {
                    n.push(e(this).prop("outerHTML"))
                }),
                n.length > 0 && (n.reverse(),
                s.html(n.join(""))),
                T()
            },
            cancel: function(i) {
    ...
    , T = function() {
        ...
        ;
        f.find(".layim-list-history").on("contextmenu", "li", function(a) {
            var l = e(this)
              , n = '<ul data-id="' + l[0].id + '" data-index="' + l.data("index") + '"><li layim-event="menuHistory" data-type="one">移除该会话</li><li layim-event="menuHistory" data-type="all">清空全部会话列表</li></ul>';
            l.hasClass("layim-null") || (t.tips(n, this, {
                tips: 1,
                time: 0,
                shift: 5,
                fix: !0,
    ...
    , j = function(i) {
        ...
        t.open({
            ...
            shift: 2,
            offset: "rb",
            content: '<img src="' + (w.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || w.base.title || "我的LayIM") + "</span>",
            success: function(i, a) {
                ...
                i.on("click", function() {
                    t.close(a),
                    f.show();
                    var i = layui.data("layim")[w.mine.id] || {};
                    delete i.close,
                    layui.data("layim", {
                        key: w.mine.id,
                        value: i
    ...
    , I = function(i) {
        ...
        if (!i.id)
            return t.msg("非法用户");
        if (a[0]) {
            var s = h.find(".layim-chat-list")
              , r = s.find(".layim-chatlist-" + i.type + i.id);
            return "none" === h.css("display") && h.show(),
            v && t.close(v.attr("times")),
            1 !== s.find("li").length || r[0] || (h.css("width", "800px"),
            s.css("display", "inline-block")),
            r[0] || (s.append(l(x).render(n)),
            a.append(l(y).render(n))),
        ...
        var d = g = t.open({
            ...
            moveType: 1,
            maxmin: !0,
            closeBtn: w.base.brief ? !1 : 1,
            content: l('<ul class="layim-chat-list">' + x + "</ul>" + y).render(n),
            success: function(a) {
                var e = layui.data("layim")[w.mine.id] || {}
                  , l = e.skin;
                h = a,
                h.css({
                    "background-image": l ? "url(" + l + ")" : "none"
                ...
                K(),
                O(i),
                layui.each(o.chatChange, function(i, a) {
                    a && a(E())
                }),
                D(),
                q(),
                a.on("click", ".layui-layim-photos", function() {
                    var i = this.src;
                    t.close(I.photosIndex),
                    t.photos({
                        photos: {
                            data: [{
                                alt: "大图模式",
    ...
    , L = function(i) {
        ...
        t.open({
            ...
            closeBtn: !1,
            shift: a.shift || 2,
            offset: e(window).height() - 50,
            content: '<img src="' + a.avatar + '"><span>' + a.name + "</span>",
            success: function(a, e) {
                i || (v = a),
                a.on("click", function() {
                    t.close(e),
                    i ? layui.each(w.chat, function(i, a) {
                        I(a)
                    }) : h.show(),
                    i && (w.chat = [],
                    _())
    ...
    , M = function(i, a) {
        ...
        var l = -1 === i.index() ? 0 : i.index()
          , n = ".layim-chat"
          , r = h.find(n).eq(l);
        if (a) {
            i.hasClass(s) && M(0 === l ? i.next() : i.prev()),
            i.remove(),
            r.remove();
            var d = h.find(n).length;
            return 1 === d && (h.find(".layim-chat-list").hide(),
            h.css("width", "600px")),
            0 === d && t.close(g),
            !1
        }
        i.addClass(s).siblings().removeClass(s),
        r.css("display", "inline-block").siblings(n).hide(),
    ...
    , q = function() {
        var i = E()
          , a = w.message[i.data.type + i.data.id];
        a && delete w.message[i.data.type + i.data.id]
    }
    , E = function() {
        var i = e(".layim-chat-list ." + s).index()
          , a = h.find(".layim-chat").eq(i)
    ...
    , O = function(i) {
        var a = layui.data("layim")[w.mine.id] || {}
          ...
          , n = t[i.type + i.id];
        if (f) {
            var s = f.find(".layim-list-history");
    ...
    , H = function() {
        var i = {
          ...
          , n = w.base.maxLength || 3e3;
        if (i.content = a.textarea.val(),
        "" !== i.content.replace(/\s/g, "")) {
            if (i.content.length > n)
                return t.msg("内容最长不能超过" + n + "个字符");
            e.append(l(b).render(i));
            var s = {
                mine: i,
                to: a.data
            }
            ...
            A(r),
            layui.each(o.sendMessage, function(i, a) {
                a && a(s)
            })
        }
        _(),
    ...
    , N = function(i) {
        ...
        A(i),
        !h && i.content || -1 === n) {
            if (w.message[i.type + i.id])
                w.message[i.type + i.id].push(i);
            else if (w.message[i.type + i.id] = [i],
            "friend" === i.type) {
                var s;
                layui.each(w.friend, function(a, e) {
                    return layui.each(e.list, function(a, e) {
                        return e.id == i.id ? (e.type = "friend",
                        e.name = e.username,
                        w.chat.push(e),
                        s = !0) : void 0
                    }),
                ...
                var o;
                layui.each(w.group, function(a, e) {
                    return e.id == i.id ? (e.type = "group",
                    e.name = e.groupname,
                    w.chat.push(e),
                    o = !0) : void 0
                }),
                ...
                w.chat.push(i);
            return "group" === i.type && layui.each(w.group, function(a, e) {
                return e.id == i.id ? (t.avatar = e.avatar,
                !0) : void 0
            }),
            L({
                name: "收到新消息",
        ...
        var r = E();
        r.data.type + r.data.id !== i.type + i.id && (a.addClass("layui-anim layer-anim-06"),
        setTimeout(function() {
            a.removeClass("layui-anim layer-anim-06")
        }, 300));
        var d = h.find(".layim-chat").eq(n)
          , u = d.find(".layim-chat-main ul");
        "" !== i.content.replace(/\s/g, "") && u.append(l(b).render(i)),
        _()
    }
    , A = function(i) {
        var a = layui.data("layim")[w.mine.id] || {}
          , e = a.chatlog || {};
        e[i.type + i.id] ? (e[i.type + i.id].push(i),
        e[i.type + i.id].length > 50 && e[i.type + i.id].shift()) : e[i.type + i.id] = [i],
        a.chatlog = e,
        layui.data("layim", {
            key: w.mine.id,
            value: a
    ...
    , D = function() {
        ...
        layui.each(e[a.data.type + a.data.id], function(i, a) {
            t.append(l(b).render(a))
        }),
        _()
    }
    , z = function(i) {
        var a, e = f.find(".layim-list-" + i.type), n = {};
        if (w[i.type])
            if ("friend" === i.type)
                layui.each(w.friend, function(e, l) {
                    return i.groupid == l.id ? (layui.each(w.friend[e].list, function(e, t) {
                        return t.id == i.id ? a = !0 : void 0
                    }),
                    a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
                        shift: 6
                    }) : (w.friend[e].list = w.friend[e].list || [],
                    n[w.friend[e].list.length] = i,
                    i.groupIndex = e,
                    w.friend[e].list.push(i),
                    !0)) : void 0
                });
            else if ("group" === i.type) {
        ...
        if (!a) {
            ...
            if ("friend" === i.type) {
                ...
                "group" === i.type && (e.append(s),
                e.find(".layim-null")[0] && e.find(".layim-null").remove())
        }
    }
    , J = function(i) {
        var a = f.find(".layim-list-" + i.type);
        w[i.type] && ("friend" === i.type ? layui.each(w.friend, function(e, t) {
            layui.each(t.list, function(t, l) {
                if (i.id == l.id) {
                    var n = a.find(">li").eq(e);
                    n.find(".layui-layim-list>li");
                    return n.find(".layui-layim-list>li").eq(t).remove(),
                    w.friend[e].list.splice(t, 1),
                    n.find(".layim-count").html(w.friend[e].list.length),
                    0 === w.friend[e].list.length && n.find(".layui-layim-list").html('<li class="layim-null">该分组下已无好友了</li>'),
                    !0
                }
            })
        }) : "group" === i.type && layui.each(w.group, function(e, t) {
            return i.id == t.id ? (a.find(">li").eq(e).remove(),
            w.group.splice(e, 1),
            0 === w.group.length && a.html('<li class="layim-null">暂无群组</li>'),
            !0) : void 0
        }))
    }
    , _ = function() {
    ...
    , F = "layui-anim-up", Y = {
        status: function(i, a) {
            var t = function() {
                i.next().hide().removeClass(F)
            }
              , l = i.attr("lay-type");
            if ("show" === l)
                $(a),
                i.next().show().addClass(F),
                e(document).off("click", t).on("click", t);
            else {
                var n = i.parent().prev();
                i.addClass(s).siblings().removeClass(s),
                n.html(i.find("cite").html()),
                n.removeClass("layim-status-" + ("online" === l ? "hide" : "online")).addClass("layim-status-" + l),
                layui.each(o.online, function(i, a) {
                    a && a(l)
                })
            }
        },
        tab: function(i) {
            var a, e = ".layim-tab-content", t = f.find(".layui-layim-tab>li");
            "number" == typeof i ? (a = i,
            i = t.eq(a)) : a = i.index(),
            a > 2 ? t.removeClass(s) : (Y.tab.index = a,
            i.addClass(s).siblings().removeClass(s)),
            f.find(e).eq(a).addClass(n).siblings(e).removeClass(n)
        },
        spread: function(i) {
            var a = i.attr("lay-type")
              , e = "true" === a ? "false" : "true"
              , t = layui.data("layim")[w.mine.id] || {};
            i.next()["true" === a ? "removeClass" : "addClass"](n),
        ...
        search: function(i) {
            var a = f.find(".layui-layim-search")
              , e = f.find("#layui-layim-search")
              , t = a.find("input")
              , l = function(i) {
                ...
                if ("" === a)
                    ...
                    for (var l = [], n = w.friend || [], s = w.group || [], o = "", r = 0; r < n.length; r++)
                        for (var d = 0; d < (n[r].list || []).length; d++)
                            -1 !== n[r].list[d].username.indexOf(a) && (n[r].list[d].type = "friend",
                            n[r].list[d].index = r,
                            n[r].list[d].list = d,
                            l.push(n[r].list[d]));
                    for (var u = 0; u < s.length; u++)
                        -1 !== s[u].groupname.indexOf(a) && (s[u].type = "group",
                        s[u].index = u,
                        s[u].list = u,
                        l.push(s[u]));
                    if (l.length > 0)
                        for (var c = 0; c < l.length; c++)
                            o += '<li layim-event="chat" data-type="' + l[c].type + '" data-index="' + l[c].index + '" data-list="' + l[c].list + '"><img src="' + l[c].avatar + '"><span>' + (l[c].username || l[c].groupname || "佚名") + "</span><p>" + (l[c].remark || l[c].sign || "") + "</p></li>";
                    else
            ...
            ;
            !w.base.isfriend && w.base.isgroup ? Y.tab.index = 1 : w.base.isfriend || w.base.isgroup || (Y.tab.index = 2),
            a.show(),
            t.focus(),
            t.off("keyup", l).on("keyup", l)
        },
        ...
        chat: function(i) {
            var a = layui.data("layim")[w.mine.id] || {}
              ...
              , n = {};
            "friend" === e ? n = w[e][t].list[l] : "group" === e ? n = w[e][l] : "history" === e && (n = (a.history || {})[t] || {}),
        ...
        groupMembers: function(i, a) {
            var l = i.find(".layui-icon")
              , n = function() {
                l.html("&#xe61a;"),
                i.data("down", null ),
                t.close(Y.groupMembers.index)
            }
              , s = function(i) {
                $(i)
            }
            ;
            i.data("down") ? n() : (l.html("&#xe619;"),
            i.data("down", !0),
            Y.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
                tips: 3,
                time: 0,
                shift: 5,
                fix: !0,
                ...
                success: function(a) {
                    ...
                    k(t, function(e) {
                        layui.each(e.list, function(i, a) {
                            n += '<li><a><img src="' + a.avatar + '"></a><p>' + a.username + "</p></li>"
                        }),
                        a.find(".layim-members-list").html(n),
                        layui.each(o.members, function(i, a) {
                            a && a(e)
                        }),
            ...
            e(document).off("mousedown", n).on("mousedown", n),
            e(window).off("resize", n).on("resize", n),
        ...
        setSend: function(i, a) {
            var t = i.siblings(".layim-menu-box")
              , l = function() {
                t.hide().removeClass(F)
            }
              , n = i.attr("lay-type");
            if ("show" === n)
                $(a),
                t.show().addClass(F),
                e(document).off("click", l).on("click", l);
            else {
                i.addClass(s).siblings().removeClass(s);
                var o = layui.data("layim")[w.mine.id] || {};
                o.sendHotKey = n,
                layui.data("layim", {
                    key: w.mine.id,
                    value: o
        ...
        face: function(i, a) {
            var l = ""
              , n = E()
              , s = function() {
                t.close(Y.face.index)
            }
            ;
            for (var o in R)
                l += '<li title="' + o + '"><img src="' + R[o] + '"></li>';
            l = '<ul class="layui-clear layim-face-list">' + l + "</ul>",
            Y.face.index = t.tips(l, i, {
                tips: 1,
                time: 0,
                fix: !0,
                ...
                success: function(i) {
                    i.find(".layim-face-list>li").on("mousedown", function(i) {
                        ...
                        B(n.textarea[0], "face" + this.title + " "),
                        t.close(Y.face.index)
                    })
                }
            }),
            ...
            layui.upload({
                ...
                success: function(i) {
                    try {
                        ...
                        return i = {},
                        t.msg("请对上传接口返回JSON字符")
                    }
                    0 == i.code ? (i.data = i.data || {},
                    "images" === a ? B(l.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === a && B(l.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]")) : t.msg(i.msg || "上传失败")
                }
            })
        },
        chatLog: function(i) {
            var a = E();
            return w.base.chatLog ? (t.close(Y.chatLog.index),
            Y.chatLog.index = t.open({
                type: 2,
                maxmin: !0,
                ...
                content: w.base.chatLog + "?id=" + a.data.id + "&type=" + a.data.type
            })) : t.msg("未开启更多聊天记录")
        },
        menuHistory: function(i, a) {
            var l = layui.data("layim")[w.mine.id] || {}
              , n = i.parent()
              , s = i.data("type")
              , o = f.find(".layim-list-history")
              , r = '<li class="layim-null">暂无历史会话</li>';
            if ("one" === s) {
                var d = l.history;
                delete d[n.data("index")],
                l.history = d,
                layui.data("layim", {
                    key: w.mine.id,
                    value: l
    ...
    i("layim", new r)
}).addcss("modules/layim/layim.css?v=2.085", "skinlayimcss");
